      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="saveTransaction">
          ...
          <div v-if="transactionData.referencia === 'Procedimento'">
            <!-- Data da Movimentação (acima do select de paciente) -->
            <label for="data">Data da Movimentação:</label>
            <input type="date" v-model="transactionData.data" @change="fetchPacientes" required />
    ...
    data() {
      return {
        transactionData: {
          ...
          valor: 0,
          data: '',
          paciente: '', // Novo campo para paciente
        },
        pacientes: [], // Lista de pacientes com agendamentos futuros
        formattedValor: '',
        loadingPacientes: false, // Novo estado para indicar que estamos carregando os pacientes
      };
    },
    methods: {
      getCurrentUser() {
...
async saveTransaction() {
  try {
    ...
    // Determine os valores de entrada, saida e caixa com base no tipo de movimentação
    let entrada = null;
    let saida = null;
    let caixa = null;
