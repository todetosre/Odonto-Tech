  ...
  <div v-if="isVisible" class="modal">
    <div class="modal-content">
      ...
      <form @submit.prevent="saveAppointment">
        ...
        <input type="text" v-model="searchTerm" @input="searchPaciente" placeholder="Digite o nome do paciente" />
        <!-- Lista de sugestões de pacientes -->
        <ul v-if="filteredPacientes.length > 0" class="suggestions">
          <li v-for="paciente in filteredPacientes" :key="paciente.id" @click="selectPaciente(paciente)">
            {{ paciente.nome }}
          </li>
        </ul>
...
export default {
  ...
  data() {
    return {
      ...
      searchTerm: '', // Para armazenar o termo de pesquisa
      filteredPacientes: [], // Lista de pacientes filtrados
      availableTimes: [
  ...
  methods: {
      ...
      if (this.searchTerm.length > 1) {
        try {
          const response = await axios.get(
            `http://localhost:3000/api/pacientes/buscar/${this.searchTerm}`
          );
    ...
    // Seleciona o paciente e preenche o campo de paciente
    selectPaciente(paciente) {
      this.appointmentData.paciente = paciente.nome;
      this.filteredPacientes = []; // Limpa a lista de sugestões
      this.searchTerm = paciente.nome; // Define o termo de pesquisa como o nome do paciente selecionado
    },
...
.modal-content h2 {
  margin-top: 0;
  /* Remove margem superior para alinhar com o topo */
  margin-bottom: 20px;
  /* Espaço inferior para separar do formulário */
}

.modal-content label {
  display: block;
  margin-bottom: 5px;
  /* Espaço entre o rótulo e o campo */
}
